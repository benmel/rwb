...
--Table with invited user's info
create table rwb_invited (
  idnum  varchar(20) not null unique, 
  email varchar(256) not null check (email LIKE '%@%'),
  referer varchar(64) not null references rwb_users(name) on delete cascade
);
--
--Table with invited user's temporary permissions
create table rwb_invited_permissions (
  idnum  varchar(20) not null references rwb_invited(idnum) on delete cascade,
  action varchar(64) not null references rwb_actions(action) on delete cascade,
  constraint temp_perm_unique unique(idnum,action)
);
