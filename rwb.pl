...
if ($action eq "base") { 
  ...
  my @rows;
  eval {
    @rows = ExecSQL($dbuser, $dbpasswd, "select unique cycle from cs339.candidate_master order by cycle","COL");
  };
  print "<div id=\"data-form\">
	<select multiple id=\"Cycle\">";

  for (my $i=0; $i < scalar @rows; $i++) {
    print "<option value=\"$rows[$i]\">$rows[$i]</option>\n";
  }
  ...
  print	"</select>
	...
	<input type=\"checkbox\" id=\"Individual\" value=\"Individual\">Individual";

  if (UserCan($user,"query-opinion-data")) {
        print "<br><input type=\"checkbox\" id=\"Opinion\" value=\"Opinion\">Opinion";
  }
...
if ($action eq "near") {
  ...
  $cyc = "1112" if (!defined($cyc) || $cyc eq "null");
  my @cycles = split(',',$cyc);
  my $cycle_query = BuildCycle(@cycles);
  ...
  if ($what{committees}) { 
    ...
    my $query_com_can_count = "select count(*) from cs339.comm_to_cand natural join cs339.cmte_id_to_geo where " . $cycle_query . " and latitude>? and latitude<? and longitude>? and longitude<?";
    my $query_com_com_count = "select count(*) from cs339.comm_to_comm natural join cs339.cmte_id_to_geo where " . $cycle_query . " and latitude>? and latitude<? and longitude>? and longitude<?";
    
    my ($latne_rev,$longne_rev,$latsw_rev,$longsw_rev) = FindCoordsCombined($query_com_can_count,$query_com_com_count,$latne,$longne,$latsw,$longsw,@cycles);
        
    my $query_com_can_party = "select sum(transaction_amnt) from cs339.comm_to_cand natural join cs339.committee_master natural join cs339.cmte_id_to_geo where " . $cycle_query . " and latitude>? and latitude<? and longitude>? and longitude<? and upper(cmte_pty_affiliation) = ?";
    my $query_com_can_total = "select sum(transaction_amnt) from cs339.comm_to_cand natural join cs339.cmte_id_to_geo where " . $cycle_query . " and latitude>? and latitude<? and longitude>? and longitude<?";
    my $query_com_com_party = "select sum(transaction_amnt) from cs339.comm_to_comm natural join cs339.committee_master natural join cs339.cmte_id_to_geo where " . $cycle_query . " and latitude>? and latitude<? and longitude>? and longitude<? and upper(cmte_pty_affiliation) = ?";
    my $query_com_com_total = "select sum(transaction_amnt) from cs339.comm_to_comm natural join cs339.cmte_id_to_geo where " . $cycle_query . " and latitude>? and latitude<? and longitude>? and longitude<?";
    
    my ($result_com_can_dem,$error1) = Aggregate($query_com_can_party,$latne_rev,$longne_rev,$latsw_rev,$longsw_rev,"DEM",@cycles);
    my ($result_com_can_rep,$error2) = Aggregate($query_com_can_party,$latne_rev,$longne_rev,$latsw_rev,$longsw_rev,"REP",@cycles);   
    my ($result_com_can_total,$error3) = Aggregate($query_com_can_total,$latne_rev,$longne_rev,$latsw_rev,$longsw_rev,undef,@cycles);
       
    my ($result_com_com_dem,$error4) = Aggregate($query_com_com_party,$latne_rev,$longne_rev,$latsw_rev,$longsw_rev,"DEM",@cycles);
    my ($result_com_com_rep,$error5) = Aggregate($query_com_com_party,$latne_rev,$longne_rev,$latsw_rev,$longsw_rev,"REP",@cycles);
    my ($result_com_com_total,$error6) = Aggregate($query_com_com_total,$latne_rev,$longne_rev,$latsw_rev,$longsw_rev,undef,@cycles);
    ...
    if (!$error1 && !$error2 && !$error3 && !$error4 && !$error5 && !$error6) {
      $dem_total = $result_com_can_dem + $result_com_com_dem;
      $rep_total = $result_com_can_rep + $result_com_com_rep;
      $dem_rep_total = $dem_total + $rep_total;
      if ($dem_rep_total == 0) {
        $dem_rep_total = 1;
      }
      $blue = int(255*($dem_total/$dem_rep_total));
      $red = int(255*($rep_total/$dem_rep_total));
      #print "<input type=\"hidden\" id=\"blue\" value=\"$blue\">";
      #print "<input type=\"hidden\" id=\"red\" value=\"$red\">";
      $total = $result_com_can_total + $result_com_com_total;
      if (($blue > 0) || ($red > 0)) {
        print "<div id=\"color-com\" style=\"height: 100%; width: 100%; background-color: rgb($red, 0, $blue); float: left\">Democrat total: \$$dem_total<br>Republican total: \$$rep_total<br>Committees total: \$$total</div>";
      } else {
        print "<div id=\"color-com\" style=\"height: 100%; width: 100%; float: left\">Committees total: \$$total</div>";
      }
    }

    #if (!$error3 && !$error6) {
    #  $total = $result_com_can_total + $result_com_com_total;
    #  print "<input type=\"hidden\" id=\"total-com\" value=\"$total\">";
    #}
  ...
  if ($what{individuals}) {
    ...
    my $query_ind_count = "select count(*) from cs339.individual natural join cs339.ind_to_geo where " . $cycle_query . " and latitude>? and latitude<? and longitude>? and longitude<?";

    my ($latne_rev,$longne_rev,$latsw_rev,$longsw_rev) = FindCoords($query_ind_count,$latne,$longne,$latsw,$longsw,@cycles);
 
    my $query_ind_total = "select sum(transaction_amnt) from cs339.individual natural join cs339.ind_to_geo where " . $cycle_query . " and latitude>? and latitude<? and longitude>? and longitude<?";

    my ($result_ind_total,$error) = Aggregate($query_ind_total,$latne_rev,$longne_rev,$latsw_rev,$longsw_rev,undef,@cycles); 
    
    if (!$error) {
      #print "<input type=\"hidden\" id=\"total-ind\" value=\"$result_ind_total\">";
      print "<div id=\"color-ind\" style=\"height: 100%; width: 100%; float: left; \">Individuals total: \$$result_ind_total</div>";
    }
  ...
  if ($what{opinions}) {
    ...
    my $query_opinion_avg = "select avg(color) from rwb_opinions where latitude>? and latitude<? and longitude>? and longitude<?";
    my $query_opinion_std = "select stddev(color) from rwb_opinions where latitude>? and latitude<? and longitude>? and longitude<?";
    my $query_opinion_count = "select count(*) from rwb_opinions where latitude>? and latitude<? and longitude>? and longitude<?";
    ...
    eval {
      @result_avg = ExecSQL($dbuser, $dbpasswd, $query_opinion_avg, "COL", $latsw, $latne, $longsw, $longne);
    };
    eval {
      @result_std = ExecSQL($dbuser, $dbpasswd, $query_opinion_std, "COL", $latsw, $latne, $longsw, $longne);
    };
    eval {
      @result_count = ExecSQL($dbuser, $dbpasswd, $query_opinion_count, "COL", $latsw, $latne, $longsw, $longne);
    };
   
   #print "<input type=\"hidden\" id=\"opinion-avg\" value=\"$result_avg[0]\">";
   #print "<input type=\"hidden\" id=\"opinion-std\" value=\"$result_std[0]\">";
   
   my $blue = int(255*(($result_avg[0]+1)/2));
   my $red = 255-$blue;
   #print "<h2>Blue:$blue Red:$red</h2>";
   #print "<input type=\"hidden\" id=\"opinion-blue\" value=\"$blue\">";
   #print "<input type=\"hidden\" id=\"opinion-red\" value=\"$red\">";
   if ($result_count[0] > 0) {
     print "<div id=\"color-opinions\" style=\"height: 100%; width: 100%; background-color: rgb($red, 0, $blue); float: left\">Opinion average: $result_avg[0]<br>Opinion standard deviation: $result_std[0]</div>";
   } else {
     print "<div id=\"color-opinions\" style=\"height: 100%; width: 100%; float: left\">Opinion average: N/A<br>Opinion standard deviation: N/A</div>";
   }
  }
}

sub FindCoords {
  my ($query,$latne,$longne,$latsw,$longsw,@cycles) = @_;
  my @result = ('0');
  my $total = $result[0];
  my ($latne_orig,$latsw_orig,$longne_orig,$longsw_orig) = ($latne+0.1,$latsw-0.1,$longne+0.1,$longsw-0.1);
  
  while ($total < 10 && $latne < $latne_orig && $latsw > $latsw_orig && $longne < $longne_orig && $longsw > $longsw_orig) {    
    eval {
      @result = ExecSQL($dbuser, $dbpasswd, $query, "COL", @cycles,$latsw,$latne,$longsw,$longne); 
    };
    $total = $result[0];
    $latne += 0.01;
    $longne += 0.01;
...
sub FindCoordsCombined {
  my ($query_com_can_count,$query_com_com_count,$latne,$longne,$latsw,$longsw,@cycles) = @_;
  my @result1 = ('0');
  my @result2 = ('0');
  my $total = $result1[0] + $result2[0];
  my ($latne_orig,$latsw_orig,$longne_orig,$longsw_orig) = ($latne+0.1,$latsw-0.1,$longne+0.1,$longsw-0.1);
  
  while ($total < 10 && $latne < $latne_orig && $latsw > $latsw_orig && $longne < $longne_orig && $longsw > $longsw_orig) {
    eval {
      @result1 = ExecSQL($dbuser, $dbpasswd, $query_com_can_count, "COL", @cycles,$latsw,$latne,$longsw,$longne); 
    };
    eval {
      @result2 = ExecSQL($dbuser, $dbpasswd, $query_com_com_count, "COL", @cycles,$latsw,$latne,$longsw,$longne);
    };
...
sub Aggregate {
  my ($sql_query,$latne,$longne,$latsw,$longsw,$party,@cycles) = @_;
  my @result;
  #my @new_cycles = @cycles[0..scalar(@cycles)-2];
  
  eval {
    if ($party) {
      @result = ExecSQL($dbuser, $dbpasswd, $sql_query, "COL",@cycles,$latsw,$latne,$longsw,$longne,$party);
    } else {
      @result = ExecSQL($dbuser, $dbpasswd, $sql_query, "COL",@cycles,$latsw,$latne,$longsw,$longne);
    }  
  };  
...
sub BuildCycle {
  ...
  for (my $i=0; $i < scalar(@cycles); $i++) {
        $temp_query = $temp_query . "cycle=? OR ";
  }

  my $cycle_query = substr($temp_query, 0, -4);
  ...
  if (!UserCan($user,"invite-users")) {
    ...
    if (!$run) { 
		print start_form(-name=>'Invite'),
		  ...
		  br;
		  print "Choose the permissions for the user:",br;
		  my @actions = ("add-users","give-cs-ind-data","give-opinion-data","invite-users","manage-users","query-cs-ind-data","query-fec-data","query-opinion-data");
		  foreach (@actions) {
		    if (UserCan($user,$_)) {
              print checkbox(-name=>$_),br;
            }
		  }        
          print submit, hr, end_form;
    } else {
      my $email=param('email');
      my $id = int(rand(10000000));
      eval { 
        ExecSQL($dbuser,$dbpasswd,'insert into rwb_invited (idnum,email,referer) values(?,?,?)',undef,$id,$email,$user);
      };
      my @actions = ("add-users","give-cs-ind-data","give-opinion-data","invite-users","manage-users","query-cs-ind-data","query-fec-data","query-opinion-data");
      foreach (@actions) {
        if (param($_)) {
          eval {
            ExecSQL($dbuser,$dbpasswd,'insert into rwb_invited_permissions (idnum,action) values(?,?)',undef,$id,$_);
          };
        }
      }
      SendEmail($email,$id);
...
sub SendEmail {
  ...
  print MAIL "To: $email\n";
  print MAIL "From: UUUUU\@gmail.com\n";
  print MAIL "Subject: RWB Invite\n\n";
  print MAIL "You have been invited to use the RWB application.\nClick on the following link to become a user:\nhttp://test.com/~UUUUU/rwb/rwb.pl?act=registration&id=$id";
  close(MAIL);
}
...
if ($action eq "registration") {
  ...
  eval {
    @count = ExecSQL($dbuser,$dbpasswd,'select count(*) from rwb_invited where idnum=?','COL',$id);
  };
  eval {
    @result1 = ExecSQL($dbuser,$dbpasswd,"select email from rwb_invited where idnum=?","COL",$id);
  }; 
  eval {
    @result2 = ExecSQL($dbuser,$dbpasswd,"select referer from rwb_invited where idnum=?","COL",$id);
  };

  my $email = $result1[0];
  my $referer = $result2[0];
  
  if (!$run) { 
	  if ($count[0] == 1) {
	    print start_form(-name=>'Registration'),
		  h2('Registration'),
		  ...
		  hidden('id',$id),
		  hidden(-name=>'run',-default=>['1']),
		  hidden(-name=>'act',-default=>['registration']),
		  submit,
		  end_form,
		  hr;
	  } else {
        print h2("You can not register.");  
      }  
  } else {
    my $name=param('name');
    my $email=param('email');
	...
	if ($error) { 
	  ...
	  #my $error = InheritPermissions($referer,$name);
	  eval {
	    ExecSQL($dbuser,$dbpasswd,"insert into rwb_permissions (name,action) select ?,action from rwb_invited_permissions where idnum=?",undef,$name,$id);
	  };
	  eval {
	    ExecSQL($dbuser,$dbpasswd,"delete from rwb_invited where idnum=?",undef,$id);
	  };
...
sub InheritPermissions {
  ...
  eval { ExecSQL($dbuser,$dbpasswd,'insert into rwb_permissions (name,action) select ?,action from rwb_permissions where name=?',undef,$invitee,$inviter);};
  return $@;
}
...
if ($action eq "give-opinion-data") { 
  ...
  my %labels = ('0','Neutral','-1','Red','1','Blue');
  if (!UserCan($user,"give-opinion-data")) {
    print h2("You do not have the required permissions to give opinion data.");
  } else {
    if (!$run) {
      print start_form(-name=>'OpinionRadio'),
        h2('Give your opinion on the political color of your location'),
          radio_group(-name=>'opinion',
	  		    -values=>['0','-1','1'],
			    -default=>'0',
			    -linebreak=>'true',
                            -labels=>\%labels),
          hidden('lat',param('lat')),
          hidden('long',param('long')),
          hidden(-name=>'run',-default=>['1']),
          hidden(-name=>'act',-default=>['registration']),
          submit,
          end_form,
          ...
          my $color = param('opinion');
          my $latitude = param('lat');
          my $longitude = param('long');
          my $error = GiveOpinion($user,$color,$latitude,$longitude);
          if ($error) {
            print "Can't add opinion because: $error.";
          } else {
            print "Your opinion was added.";
          }
        }
    }
    print "<p><a href=\"rwb.pl?act=base&run=1\">Return</a></p>";
...
#
sub Committees {
  my ($latne,$longne,$latsw,$longsw,@cycles,$format) = @_;
  my @rows;
 
  my $query1 = "select latitude, longitude, cmte_nm, cmte_pty_affiliation, cmte_st1, cmte_st2, cmte_city, cmte_st, cmte_zip from cs339.committee_master natural join cs339.cmte_id_to_geo where ";
  
  my $temp_query = "(";
  for (my $i=0; $i < scalar(@cycles)-1; $i++) {
  	$temp_query = $temp_query . "cycle=? OR ";
  }
  
  my $query2 = substr($temp_query, 0, -4);
  my $query3 =  ") and latitude>? and latitude<? and longitude>? and longitude<?";
  
  my $sql_query = $query1 . $query2 . $query3;
  ...
  my @new_cycles = @cycles[0..scalar(@cycles)-2]; 
  eval {
    @rows = ExecSQL($dbuser, $dbpasswd, $sql_query, undef,@new_cycles,$latsw,$latne,$longsw,$longne);
  };  
...
sub Candidates {
  ...
  my $query1 = "select latitude, longitude, cand_name, cand_pty_affiliation, cand_st1, cand_st2, cand_city, cand_st, cand_zip from cs339.candidate_master natural join cs339.cand_id_to_geo where ";
...
sub Individuals {
  ...
  my $query1 = "select latitude, longitude, name, city, state, zip_code, employer, transaction_amnt from cs339.individual natural join cs339.ind_to_geo where ";
